...
footprintjs's `retry`. They do — that landed in 0.32.0. What the audit found by
pointing at it was the same bug still live for three other kinds: `pause`,
`resume` and `emit` had been in the entry union, and in real engine output, the
whole time, and every one of them rendered with the generic step badge. A
paused run announced itself to a screen reader as "Data operation".

The reason the map could drift is now gone, and the retry story is pinned
against a real engine run instead of hand-built entries.

### Fixed

- **`pause`, `resume` and `emit` have their own badges.** `‖ Paused`,
  `▷ Resumed`, `◈ Emitted event` — each with the icon you see and the label a
  screen reader hears. Before, all three fell through to `· Data operation`.

- **The badge map can no longer go stale.** It was keyed by `string`, so a kind
  added to the union without a badge was invisible to the compiler; it is now
  keyed by `NarrativeEntry["type"]`, which makes the same omission a build
  error. The runtime fallback stays for a kind from a NEWER footprintjs than
  this release compiles against — that entry renders with the neutral step
  badge rather than vanishing, and there is a test that says so.

### Added

- **A golden fixture recorded from a real retry.** `retry-attempts` — a flaky
  fetch with a 3-attempt policy that fails twice and then succeeds, followed by
  a stage whose declared policy never fires. Generated by running the engine,
  like every other fixture, and it pins what a retried stage actually looks
  like downstream: two `retry` entries carrying "attempt 1 of 3" / "attempt 2
  of 3", both nested at depth 1, ONE `runtimeStageId` and ONE commit bundle
  across all three attempts — so a retried stage never multiplies the
  time-travel cursor — and the surviving attempt's write (3) in memory, not the
  discarded ones.

- **The fixture generator captures `onStageRetry`.** Retry is the one flow
  event that fires DURING a stage rather than at its boundary, so the fixture
  now carries the real interleaving (both retries before the stage's own
  `onStageExecuted`, and no `onError` at all — a stage that recovered did not
  fail).

- **The narrative kinds are documented.** A table of all thirteen with their
  icon, label and meaning, plus the `data-type` attribute that lets unstyled
  mode reach any of them by selector. Previously a reader could not discover
  from the docs that `retry` existed.

### Changed

- **footprintjs devDependency 9.5.0 → 9.15.1** (generator-only; the published
  library still has no footprintjs dependency). All four existing fixtures were
  re-recorded on the new engine and every golden output snapshot was unchanged
  — the only fixture drift is additive fields the engine gained meanwhile
  (`parentRuntimeStageId`, `loopIteration`).

### Honest note

An asymmetry upstream, pinned by a test so nothing here gets built on the wrong
half of it: a structure event carries `spec.retryAttempts` when the policy is
declared as a stage OPTION, but NOT when it arrives via the `.retry()`
modifier, because the modifier is chained after the stage has already been
added and the event has already fired. Structure events are a lower bound on
which stages can retry; the built spec is the full answer. Any future
"this stage can retry" badge on the chart must read the spec.
